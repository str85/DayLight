...
Операции добавления средств миллиарду граждан будут очень ресурсозатратными, поэтому их нужно делать в несколько итераций в течение нескольких часов.
...
Если в настройках параметров государства выбрана мультипартийная политическая система, тогда любой гражданин достигший определенного в параметрах возраста может создать свою партию.
...
Глава государства имеет право устанавливать размер налогов, загружать в блокчейн бюджет страны. Но при изменении настроек параметров гос-ва различные функции могут быть переданы другим органам власти.
...
Дата рождения, и местонахождение необходимы для участие в выборах в органы власти.
...
В результате этой транзакции в таблице Users status становится равен "умер", в результате чего становится возможным выполнение смарт-контракта, распределяющего наследство. Если такого смарт-контракта нет, тогда все средства и имущество умершего гражданина автоматически распределяется между его самыми близкими родственниками.
...
Владалец недвижимости может иметь мультиподпись и хранить 2-й ключ в специальном органе, задача которого не допустить насильственного воздействия на граждан с целью кражи их имущества. В таком случае, если Владалец недвижимости захочет совершить сделку, то ему нужно будет прийти в такое учреждение, назвать ID объекта недвижимости, подписать своим первым ключом спец. идентификационной запрос, после чего он получит подпись, отправив её в сеть сделка будет совершена. Если продавец не боится, то может отключить необходимость 2-й подписи.
...
###### PropertyRequestsAcceptHistory (История подтверждений запросов на недвижимость)
...
В случае смерти гражданина, становится возможным расшифровка и выполнение его смарт-контракта, в котором описан алгоритм распределения наследства. 
...
В самом начале идет проверка статуса в таблице Spouses. Если там стоит "разведены", тогда происходит выполнение контракта. Например, все акции могут остаться за мужем, а недвижимость перейдет к жене.
...
Все обменные операции будут происходить на внутренней децентрализованной бирже. Покупать и продавать валюту смогут как пользователи так и компании на равных условиях.
